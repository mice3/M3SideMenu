...
#import <M3SideMenu/M3SideMenu.h>
...
- (void)viewDidLoad {
    ...
    UINib *sideMenuCellNib = [UINib nibWithNibName:@"M3SideMenuCell" bundle:[NSBundle mainBundle]];
    [self.menu.tableView registerNib:sideMenuCellNib forCellReuseIdentifier:kCellIdentifier];
...
- (void)buttonClickHandler:(UIButton *)aButton {
    if (aButton.tag == 1) {
        [self.menu addButton:nil];
    } else {
        [self.menu removeButton:aButton];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row != [self.tableObjects count]) {
        ...
        description = [NSString stringWithFormat:@"Delete Cell %@", [dict objectForKey:@"name"]];
